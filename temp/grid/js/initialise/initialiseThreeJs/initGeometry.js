  ...
  function getTextureMaping(orderline) {
    ...
    function prepareContainer(containerItem, order) {  
        ...
        for (var key in order.orderlinelist.orderline) if (order.orderlinelist.orderline.hasOwnProperty(key)) {
            ...
            initmesh = new THREE.Mesh(containerItem.geometry, containerItem.material);
            initmesh.position.set(containertype.offset.x, containertype.offset.y, containertype.offset.z);
            //todo: BUG move to another place !!!dont change cas and receive shadow!!!
            // mesh.castShadow = true;
            // mesh.receiveShadow = true;
            container.mesh.add( initmesh );   
